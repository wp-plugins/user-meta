...
class umFieldBuilder {
    ...
    private function _element_input_type( $args ) {
        ...
        $args['options'] = $this->inputs['input_type']['options'];
        
        $input = new umInput( 'select', $args );
        
        $input->setOptionAttr( array(
            'email'     => 'data-child="retype_email,retype_label"',
            'password'  => 'data-child="retype_password,retype_label"'
        ) );
    ...
    function fieldSpecification() {
        ...
        $fields = array(
            ...
            'user_email' => array(
                'basic'     => array_merge( $start2, array(array('checkbox_group', '', 'retype_email'), 'retype_label') ),
                'advanced'  => array_merge( $checkbox1, $style1 ),
            ),
            'user_pass' => array(
                'basic'     => array_merge( $start2, array('regex', 'error_text'),
                        array(array('checkbox_group', '', 'retype_password', 'password_strength', 'required_current_password'), 'retype_label') ),
                'advanced'  => array_merge( array(array('checkbox_group', 'Rule', 'admin_only')), $style1 ),
            ),
            'description' => array(
        ...
        if ( $userMeta->isPro ) {
            $fieldsPro = array(
                ...
                'datetime' => array(
                    'basic'     => array_merge( $start4, array( 'datetime_selection', 'date_format', 'year_range', 'required' ) ),
                ...
                'custom' => array(
                    'basic'     => array_merge( $start4, array( 'input_type', 'regex', 'error_text', 'retype_label', 'required', 'retype_email', 'retype_password' ) ),
                    'advanced'  => array_merge( $checkbox2, $style3 ),
                ),
                ...
                'captcha' => array(
                    'basic'     => array_merge( $start1, array( 'captcha_theme' ), array(array('checkbox_group', '', 'registration_only')) ),
                    'advanced'  => array()
                ),
            );
