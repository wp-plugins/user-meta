...
/* 
* Self containing class, no dependency on userMeta class
* use for importing user
*/
if(!class_exists('userMetaImport')){
    class userMetaImport {
        ...
        function admin_menu(){
       	    add_submenu_page( 'users.php', 'User Import', 'User Import', 'manage_options', 'user-meta-import', array( __CLASS__, 'userImport'));             
        }
        ...
        function userImport(){
        	...
        	if ($_POST['mode'] == "submit") {
                ...
                if(!$_FILES['csv_file']['tmp_name'])                       
                     self::show_messages( 'It seems the file was not uploaded correctly.', 'error' );              
                ...
                if( !in_array('username',$meta_keys) ) 
                    self::show_messages( 'username field should be appear at first row in CSV file.', 'error' );   
                
                //Extend time limit for extending execution time    
                set_time_limit(3600);         
                ...
                fclose($file);
                self::show_messages( "CSV file have been imported successfully. User added: $user_added, User updated: $user_updated, User skipped: $user_skipped" );			
        	
            } 	// end of 'if mode is submit'
        
            self::inputForm($html_update); 
